            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-955 border-b border-zinc-200 dark:border-zinc-800 text-zinc-500 uppercase tracking-wider font-semibold">
                  <th className="py-4 px-6">{t("users.name")}</th>
                  <th className="py-4 px-6">{t("users.email")}</th>
                  <th className="py-4 px-6">{t("users.role")}</th>
                ...
                {users.map((item: any) => (
                  <tr key={item.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50">
                    ...
                    <td className="py-4 px-6 text-center flex items-center justify-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          if (confirm(t("users.confirmDelete", { name: item.name }))) {
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="px-6 py-4 border-b border-zinc-150 dark:border-zinc-800 flex items-center justify-between">
              <h3 className="text-md font-bold text-zinc-900 dark:text-white">
                {editingUser ? t("users.editTitle") : t("users.addTitle")}
              </h3>
              <button onClick={() => setIsOpen(false)} className="text-zinc-400 hover:text-zinc-650 text-lg font-bold">&times;</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid gap-1">
                <label className="text-xs font-semibold text-zinc-600 dark:text-zinc-300">{t("users.name")}</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid gap-1">
                <label className="text-xs font-semibold text-zinc-600 dark:text-zinc-300">{t("users.email")}</label>
                <input
                  type="email"
                  required
              ...
              <div className="grid gap-1">
                <label className="text-xs font-semibold text-zinc-600 dark:text-zinc-300">{t("users.password")}</label>
                <input
                  type="password"
              ...
              <div className="grid gap-1">
                <label className="text-xs font-semibold text-zinc-600 dark:text-zinc-300">{t("users.role")}</label>
                <select
                  value={role}
                ...
                {editingUser && (
                  <span className="text-[10px] text-zinc-400 italic">{t("users.roleLockedNote")}</span>
                )}
              </div>
